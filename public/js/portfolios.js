...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 2. CALCULADORA DE INPUT (Validación en tiempo real)
    ...
    if (investInput) {
        ...
        investInput.addEventListener('input', (e) => {
            ...
            // Leemos el ticket mínimo del dataset (guardado al abrir el modal)
    ...
    await loadAllPortfolios(); // Carga TODOS los fondos
    
    // 4. LISTENERS DE FORMULARIOS
    setupFormListeners(token);
});

// --- CARGA DE PORTAFOLIOS (SIN LÍMITE) ---
async function loadAllPortfolios() {
    try {
        ...
        data.forEach(p => {
            ...
            // HTML DE LA TARJETA (Idéntico al Dashboard)
            grid.innerHTML += `
        ...
        console.error(e); 
        const grid = document.getElementById('portfolio-grid');
        if(grid) grid.innerHTML = '<p class="col-span-full text-center text-red-500">Error cargando el mercado.</p>';
    }
}

// --- HELPERS Y LÓGICA DE MODALES ---
...
async function updateUserData(token) { 
    try { 
        ...
        if(r.ok) {
            ...
            const b = document.getElementById('modal-balance-display'); 
            if(b) b.innerText = fmt.format(d.availableBalance);
        }
    } catch(e){} 
}
    ...
    if(btnFinalConfirm) btnFinalConfirm.addEventListener('click', async () => {
        ...
        try {
            ...
            if(res.ok) { 
                ...
                loadAllPortfolios(); // Recargar tarjetas
                showSuccess(); // Mostrar modal verde
            } else { 
                alert(data.message); 
                backToStep1(); 
            }
...
window.setupInvest = function(id, name, ticket) {
    ...
    investModal.dataset.ticket = ticket; // Guardar ticket
