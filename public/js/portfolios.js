...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (investInput) investInput.addEventListener('input', (e) => {
        ...
        else { const p = val/min; calcMsg.innerText = `Ocuparás ${p} Cupo${p>1?'s':''}`; calcMsg.className = "text-xs font-bold text-emerald-500 text-right mt-1"; if(btnContinue) btnContinue.disabled=false; }
    });

    // Carga de Datos
    await updateUserData(token);
    await loadAllPortfolios(); // Esperar carga
...
async function loadAllPortfolios() {
    ...
    if(!grid) return;
    grid.innerHTML = '<p class="col-span-full text-center py-10 text-slate-400">Cargando...</p>';
    ...
    try {
        ...
        grid.innerHTML = ''; 
        if(data.length === 0) { grid.innerHTML = '<p>No hay portafolios.</p>'; return; }
        ...
        data.forEach(p => {
            ...
            const percentage = Math.min(100, (soldTickets / totalTickets) * 100);
            ...
            <div class="bg-white dark:bg-card-dark border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm flex flex-col h-full group hover:shadow-lg transition-all duration-300">
                <div class="flex justify-between mb-3 items-start">
                    ...
                    <div class="flex flex-col items-end">
                        <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase ${riskColor} leading-none">Riesgo ${p.risk}</span>
    ...
    } catch(e) { console.error(e); grid.innerHTML = '<p class="col-span-full text-center text-red-500">Error de conexión.</p>'; }
}

// HELPERS Y FORMULARIOS (Iguales a Dashboard)
