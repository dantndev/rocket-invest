...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 2. INPUTS & CALCULADORA
    initInputs();

    // 3. CARGA DE DATOS
    await updateUserData(token);
    await loadPortfolios(); // Esperar a que carguen
    renderMarketChart();
    ...
    if (investInput && calcMsg) {
        investInput.addEventListener('input', (e) => {
            ...
            if (!val || val < 1000) {
                ...
                calcMsg.innerText = `${parts} Participación${parts > 1 ? 'es' : ''}`;
...
async function loadPortfolios() {
    try {
        ...
        // SOLO 3 PARA DASHBOARD
        data.slice(0, 3).forEach(p => {
            const investors = p.investors || 0;
            const target = p.targetInvestors || 5000;
            const spotsLeft = Math.max(0, Math.ceil((p.targetAmount - p.currentAmount)/1000)); // Calculo seguro
...
function setupInvestForm(token) {
    ...
    if(btnFinalConfirm) btnFinalConfirm.addEventListener('click', async () => {
        ...
        try {
            ...
            // Leer respuesta como JSON siempre
            const data = await res.json();
