...
window.initStripePayment = async function() {
    ...
    try {
        ...
        if(summaryEl) summaryEl.innerText = fmt.format(amount);
    ...
    window.myChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderWidth: 2, pointRadius: 3, pointHitRadius: 20, fill: true, tension: 0.3
            }]
        },
        options: {
            responsive: true,
...
function initCalculator() {
    ...
    inp.addEventListener('input', (e)=>{
        ...
        else { msg.innerText=`Adquiriendo ${v/min} Ticket${(v/min)>1?'s':''}`; msg.classList.remove('text-red-500','text-orange-500'); if(btn) btn.disabled=false; }
    });
}
...
function updateBalanceUI(d) { 
    ...
    s('display-net-worth',`${f.format(d.netWorth)} <span class="text-2xl font-normal">MXN</span>`); 
    ...
    if (greetingEl) {
        // Tomar el primer nombre o usar "Inversor" si no hay datos
        const name = d.first_name ? d.first_name.split(' ')[0] : 'Inversor';
        greetingEl.innerText = `Hola, ${name}`;
    }
}

// GLOBALES
window.setupInvest = function(id,n,t) { if(!investModal) return; investModalTitle.innerText=n; investModalIdInput.value=id; investModal.dataset.ticket=t; window.backToStep1(); investModal.classList.remove('hidden'); setTimeout(() => { investModal.classList.remove('opacity-0'); investModal.querySelector('div').classList.add('scale-100'); }, 10); const inp = document.getElementById('invest-amount'); if(inp) { inp.value = ''; inp.placeholder = `Ej. ${t}`; inp.step = t; inp.min = t; } const msg = document.getElementById('invest-calculation'); if(msg) msg.innerText = ''; }
