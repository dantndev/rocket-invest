...
function setupFormListeners(token) {
    ...
    if(btnFinalConfirm) btnFinalConfirm.addEventListener('click', async () => {
        ...
        try {
            ...
            const data = await res.json(); // Parseamos siempre primero

            if(res.ok) { 
                closeInvestModal(); 
                updateUserData(token); 
                loadPortfolios(); // Llamada correcta
                alert("¡Inversión realizada con éxito! 🚀"); // MENSAJE DE ÉXITO
            } else { 
                alert(data.message); 
                backToStep1(); 
            }
