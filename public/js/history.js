        ...
        data.forEach(tx => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-slate-50 dark:hover:bg-white/5 border-b border-slate-100 dark:border-slate-700">
                    ...
                    <td class="p-4 text-sm truncate max-w-xs dark:text-white">${tx.description}</td>
                    <td class="p-4 text-right font-bold ${isPos?'text-emerald-500':'text-slate-700'}">${isPos?'+':''}${fmt.format(tx.amount)}</td>
                    <td class="p-4 text-center"><button onclick="window.downloadPDF(${tx.id})" class="text-slate-400 hover:text-primary p-2"><span class="material-symbols-outlined">description</span></button></td>
                </tr>`;
        });
