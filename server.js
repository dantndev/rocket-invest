...
// 1. ADMIN & PARTNERS
app.get('/api/admin/stats', async (req, r) => {
    const t = req.headers.authorization?.split(' ')[1]; if (!t) return r.status(401).json({});
    try {
        const d = jwt.verify(t, SECRET_KEY);
        if (d.email !== ADMIN_EMAIL) return r.status(403).json({});
        ...
        const l = await query('SELECT id,email,balance,first_name,last_name FROM users ORDER BY id DESC LIMIT 50');
        r.json({ totalUsers: parseInt(u.rows[0].c), totalAUM: (parseFloat(b.rows[0].t || 0) + parseFloat(i.rows[0].t || 0)), users: l.rows });
    } catch { r.status(500).json({}); }
});

app.post('/api/partner/stats', async (req, r) => {
    const { providerName } = req.body;
    try {
        const mf = portfoliosBase.filter(p => p.provider === providerName);
        if (mf.length === 0) return r.status(404).json({});
        let tr = 0, ti = 0; const fd = [];
        await Promise.all(mf.map(async p => {
            ...
            const ra = p.baseAmount + parseFloat(s.rows[0].t || 0);
            const inu = p.baseInvestors + parseInt(c.rows[0].co || 0);
            tr += ra; ti += inu;
            fd.push({ id: p.id, name: p.name, raised: ra, target: p.targetAmount, progress: Math.min(100, (ra / p.targetAmount) * 100), investors: inu });
        }));
        r.json({ provider: providerName, totalRaised: tr, totalInvestors: ti, activeFunds: mf.length, funds: fd });
    } catch { r.status(500).json({}); }
});

app.post('/api/partner/request', async (req, r) => {
    const { providerName, fundName, ticker, targetAmount, description } = req.body;
    try { await query('INSERT INTO fund_requests (providerName,fundName,ticker,targetAmount,description,date) VALUES ($1,$2,$3,$4,$5,$6)', [providerName, fundName, ticker, targetAmount, description, new Date().toISOString()]); r.json({ message: 'OK' }); } catch { r.status(500).json({}); }
});
...
app.get('/api/portfolios', async (req, res) => {
    try {
        ...
        res.json(l);
    } catch { res.json(portfoliosBase); }
});

app.post('/api/invest', async (req, r) => {
    const { portfolioId, amount, token } = req.body;
    try {
        const d = jwt.verify(token, SECRET_KEY);
        const u = (await query('SELECT * FROM users WHERE id=$1', [d.id])).rows[0];
        const ia = parseFloat(amount);
        const p = portfoliosBase.find(x => x.id === parseInt(portfolioId));
        
        if (ia < p.minInvestment || ia % p.minInvestment !== 0) return r.status(400).json({ message: `Múltiplos de $${p.minInvestment}` });
        if (parseFloat(u.balance) < ia) return r.status(400).json({ message: 'Saldo insuficiente' });
        
        const e = await query('SELECT id FROM investments WHERE userId=$1 AND portfolioId=$2', [u.id, p.id]);
        if (e.rows.length > 0) return r.status(400).json({ message: 'Ya eres socio' });

        await query('UPDATE users SET balance=balance-$1 WHERE id=$2', [ia, u.id]);
        await query('INSERT INTO investments (userId,portfolioId,amount,date) VALUES ($1,$2,$3,$4)', [u.id, p.id, ia, new Date().toISOString()]);
        const s = ia / p.minInvestment;
        await query('INSERT INTO transactions (userId,type,description,amount,date) VALUES ($1,$2,$3,$4,$5)', [u.id, 'invest', `Compra ${s} cupos ${p.name}`, -ia, new Date().toISOString()]);
        const nu = await query('SELECT balance FROM users WHERE id=$1', [u.id]);
        await sendEmail(u.email, 'Inversión Confirmada', `<h1>Has invertido $${ia}</h1>`);
        r.status(201).json({ message: 'Exito', newBalance: parseFloat(nu.rows[0].balance) });
    } catch (e) { console.error(e); r.status(500).json({ message: 'Error' }); }
});

app.get('/api/auth/me', async (req, r) => {
    try {
        const t = req.headers.authorization?.split(' ')[1]; if (!t) return r.status(401).send();
        const d = jwt.verify(t, SECRET_KEY);
        const u = (await query('SELECT * FROM users WHERE id=$1', [d.id])).rows[0];
        const i = await query('SELECT amount FROM investments WHERE userId=$1', [u.id]);
        let ti = 0, tc = 0;
        i.rows.forEach(x => { ti += parseFloat(x.amount); tc += parseFloat(x.amount) * 1.015; });
        r.json({ id: u.id, email: u.email, first_name: u.first_name, last_name: u.last_name, kyc_status: u.kyc_status, availableBalance: parseFloat(u.balance), investedAmount: ti, currentValue: tc, profit: tc - ti, netWorth: parseFloat(u.balance) + tc });
    } catch { r.status(401).send(); }
});

// STRIPE
app.post('/api/create-payment-intent', async (req, r) => {
    const { amount, token } = req.body;
    try {
        jwt.verify(token, SECRET_KEY);
        const p = await stripe.paymentIntents.create({ amount: Math.round(parseFloat(amount) * 100), currency: 'mxn', automatic_payment_methods: { enabled: true } });
        r.json({ clientSecret: p.client_secret });
    } catch (e) { r.status(500).json({ error: e.message }); }
});

app.post('/api/deposit', async (req, r) => {
    const { amount, token } = req.body;
    try {
        const d = jwt.verify(token, SECRET_KEY);
        const u = (await query('SELECT * FROM users WHERE id=$1', [d.id])).rows[0];
        const a = parseFloat(amount);
        await query('UPDATE users SET balance=balance+$1 WHERE id=$2', [a, u.id]);
        await query('INSERT INTO transactions (userId,type,description,amount,date) VALUES ($1,$2,$3,$4,$5)', [u.id, 'deposit', 'Depósito', a, new Date().toISOString()]);
        r.status(201).json({ message: 'OK' });
    } catch { r.status(500).send(); }
});
