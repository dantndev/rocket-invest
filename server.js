    ...
    try {
        const livePortfolios = await Promise.all(portfoliosBase.map(async (p) => {
            ...
            return {
                ...
                // Enviamos estos para que el frontend no falle si faltan
                targetInvestors: Math.floor(p.targetAmount / p.minInvestment),
                currentInvestors: p.baseInvestors + realInvestors // Alias
            };
        }));
        res.json(livePortfolios);
    } catch (error) {
        console.error(error);
        res.json(portfoliosBase); // Fallback
    }
});
...
app.post('/api/withdraw', async (req, res) => {
    ...
    try {
        ...
        const user = userRes.rows[0];
        const val = parseFloat(amount);
        if (val <= 0 || user.balance < val) return res.status(400).json({ message: 'Fondos insuficientes' });
        await query('UPDATE users SET balance = balance - $1 WHERE id = $2', [val, user.id]);
...
app.post('/api/auth/register', async (req, res) => { /* Register igual */
...
app.post('/api/auth/login', async (req, res) => { /* Login igual */
    const { email, password } = req.body;
    try {
...
// Rutas extras para que no falle my-investments y transactions
app.get('/api/my-investments', async (req, res) => { /* ... copia la del mensaje anterior si falta, pero la puse arriba */ });
app.get('/api/transactions', async (req, res) => { /* ... copia la del mensaje anterior ... */ });
