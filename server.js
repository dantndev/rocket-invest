...
// INICIALIZAR DB Y TABLA DE SOLICITUDES
async function initApp() {
    ...
    try {
        ...
        `);
        console.log("✅ Tabla fund_requests lista.");
    } catch (e) { console.error("Error tabla requests:", e); }
}
initApp();
...
const portfoliosBase = [
    ...
    { id: 3, name: "Energía Limpia Global", provider: "iShares", ticker: "ICLN", risk: "Medio", returnYTD: 14.50, targetAmount: 15000000, baseAmount: 0, baseInvestors: 0, minInvestment: 5000, lockUpPeriod: "24 Meses", description: "Infraestructura renovable global." },
...
// --- RUTAS PARTNERS (NUEVO) ---

// 1. Obtener estadísticas de un Proveedor (Simulando sesión de Partner)
app.post('/api/partner/stats', async (req, res) => {
    const { providerName } = req.body; // Ej: "BlackRock"
    try {
        // Filtramos solo los fondos de este proveedor
        ...
        // Calculamos datos reales para sus fondos
        ...
        res.json({
            ...
            totalRaised,
            totalInvestors,
            activeFunds: myFunds.length,
            funds: fundsDetails
        });

    } catch (error) { console.error(error); res.status(500).json({ message: 'Error Partner API' }); }
});

// 2. Solicitar Nuevo Fondo
app.post('/api/partner/request', async (req, res) => {
    const { providerName, fundName, ticker, targetAmount, description } = req.body;
    try {
        await query(
            'INSERT INTO fund_requests (providerName, fundName, ticker, targetAmount, description, date) VALUES ($1, $2, $3, $4, $5, $6)',
            [providerName, fundName, ticker, targetAmount, description, new Date().toISOString()]
        );
        res.json({ message: 'Solicitud enviada a Administración' });
    } catch (e) { console.error(e); res.status(500).json({ message: 'Error guardando solicitud' }); }
});


// --- RUTAS NORMALES (Mantenidas igual) ---
