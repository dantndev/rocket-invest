...
// DATOS ESTÁTICOS (Los portafolios no cambian mucho, los dejamos aquí por simplicidad)
// server.js

// Base de datos temporal de Portafolios (EXPANDIDA)
const portfolios = [
...
// 2. Obtener Datos del Usuario (Perfil y Saldo REAL desde BD)
app.get('/api/auth/me', async (req, res) => {
    ...
    try {
        ...
        // SQL: SELECT busca al usuario en el archivo
...
// 3. Mis Inversiones (Leídas desde BD)
app.get('/api/my-investments', async (req, res) => {
    ...
    try {
        ...
        // SQL: Traeme todas las inversiones de este ID
        const userInvestments = await db.all('SELECT * FROM investments WHERE userId = ?', [decoded.id]);

        // Cruzamos los datos con la lista de portafolios para saber el nombre
...
// 4. Procesar Inversión (Guardar en BD y Restar Saldo)
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        // Insertar nuevo usuario con saldo inicial
        await db.run(
            'INSERT INTO users (email, password, balance) VALUES (?, ?, ?)',
            [email, hashedPassword, 50000]
        );
...
// [NUEVO] 7. Depositar Fondos (Simulación de Pago)
app.post('/api/deposit', async (req, res) => {
    const { amount, token } = req.body; // No guardamos datos de tarjeta por seguridad en MVP
    ...
    try {
        ...
        console.log(`💳 Depósito de $${depositAmount} recibido. Nuevo saldo: $${updatedUser.balance}`);
