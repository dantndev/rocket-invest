...
// 1. GENERADOR DE PDF (NUEVO)
app.get('/api/contract/:id', async (req, res) => {
    // Validar Token desde Query Param (?token=...) porque es una descarga directa
    ...
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=Comprobante_RocketInvest_${txId}.pdf`);

        doc.pipe(res); // Enviar directo al cliente

        // DISEÑO DEL PDF
        doc.fontSize(20).font('Helvetica-Bold').text('ROCKETINVEST', { align: 'center' });
        doc.fontSize(10).font('Helvetica').text('Plataforma de Inversión Colectiva S.A.P.I. de C.V.', { align: 'center' });
        doc.moveDown();
        doc.moveDown();

        doc.fontSize(16).text('COMPROBANTE DE OPERACIÓN', { align: 'center' });
        doc.moveDown();

        doc.fontSize(12).text(`Fecha de Emisión: ${new Date().toLocaleString()}`);
        doc.text(`Folio de Operación: #${tx.id}`);
        doc.text(`Usuario: ${email}`);
        doc.moveDown();

        // Caja de Detalles
        doc.rect(50, 230, 500, 100).stroke();
        doc.text('DETALLES DEL MOVIMIENTO', 60, 240);
        doc.moveDown(0.5);
        
        const typeMap = { 'invest': 'Inversión', 'deposit': 'Depósito', 'withdraw': 'Retiro', 'sell': 'Venta' };
        doc.text(`Tipo: ${typeMap[tx.type] || tx.type}`, 60, 265);
        doc.text(`Concepto: ${tx.description}`, 60, 285);
        doc.font('Helvetica-Bold').text(`Monto: $${parseFloat(tx.amount).toLocaleString('es-MX')} MXN`, 60, 305);

        doc.moveDown(5);
        doc.font('Helvetica').fontSize(8).text('Sello Digital de Autenticidad:', { align: 'center' });
        doc.font('Courier').text(`${bcrypt.hashSync(tx.date + tx.amount, 1).substring(10, 50)}...`, { align: 'center' });
        
        doc.moveDown();
        doc.font('Helvetica').text('Este documento es un comprobante digital generado por RocketInvest Platform. Para aclaraciones contacte a soporte.', { align: 'center' });
...
// RESTO DE RUTAS (Iguales, solo asegúrate de que estén)
