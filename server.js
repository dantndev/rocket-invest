...
app.get('/api/portfolios', async (req, res) => {
    try {
        const livePortfolios = await Promise.all(portfoliosBase.map(async (p) => {
            ...
            return { ...p, currentAmount: p.baseAmount + realMoney, currentInvestors: p.baseInvestors + realInvestors, investors: p.baseInvestors + realInvestors };
        }));
        res.json(livePortfolios);
    } catch (error) { console.error(error); res.json(portfoliosBase.map(p => ({...p, currentAmount: p.baseAmount, currentInvestors: p.baseInvestors, investors: p.baseInvestors}))); }
});
...
app.get('/api/my-investments', async (req, res) => {
    ...
    try {
        ...
        const enriched = result.rows.map(inv => {
            ...
            return { id: inv.id, portfolioName: portfolio ? portfolio.name : 'Fondo Desconocido', risk: portfolio ? portfolio.risk : 'N/A', investedAmount: amount, currentValue: currentVal, profit: currentVal - amount, date: inv.date };
        });
        res.json(enriched);
...
app.post('/api/invest', async (req, res) => {
    ...
    try {
        ...
        await query('INSERT INTO transactions (userId, type, description, amount, date) VALUES ($1, $2, $3, $4, $5)', [user.id, 'invest', `Entrada a grupo ${portfolio.name}`, -investmentAmount, new Date().toISOString()]);
...
app.post('/api/deposit', async (req, res) => { /* Mismo código de siempre */
...
app.post('/api/withdraw', async (req, res) => { /* Mismo código de siempre */
    const { amount, token } = req.body;
    try {
        const decoded = jwt.verify(token, SECRET_KEY);
        const userRes = await query('SELECT * FROM users WHERE id = $1', [decoded.id]);
...
app.post('/api/sell', async (req, res) => { /* Mismo código de siempre */
    ...
    try {
        ...
        const portfolio = portfoliosBase.find(p => p.id === investment.portfolioid); 
...
app.get('/api/transactions', async (req, res) => { /* Mismo código de siempre */
    const token = req.headers.authorization?.split(' ')[1];
    if (!token) return res.status(401).json({ message: 'No token' });
    try {
        const decoded = jwt.verify(token, SECRET_KEY);
...
app.post('/api/auth/register', async (req, res) => { /* Mismo código de siempre */
    ...
    try {
        ...
        await query('INSERT INTO transactions (userId, type, description, amount, date) VALUES ($1, $2, $3, $4, $5)', [result.rows[0].id, 'deposit', 'Bono de Bienvenida', 50000, new Date().toISOString()]);
        res.status(201).json({ message: 'Creado' });
    } catch (e) { console.error(e); res.status(500).json({ message: 'Error' }); }
});

app.post('/api/auth/login', async (req, res) => { /* Mismo código de siempre */
    const { email, password } = req.body;
    try {
