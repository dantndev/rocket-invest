...
async function initApp() {
    ...
    try {
        ...
        console.log("✅ Base de datos lista con Nombres y KYC.");
...
// REGISTRO CON NOMBRE (KYC NIVEL 1)
app.post('/api/auth/register', async (req, res) => {
    const { email, password, first_name, last_name } = req.body;
    try {
        const existing = await query('SELECT id FROM users WHERE email = $1', [email]);
        if (existing.rows.length > 0) return res.status(400).json({ message: 'El correo ya está registrado.' });

        const hashed = await bcrypt.hash(password, 10);
        
        // Insertar con Nombres
        ...
        await sendEmail(email, 'Bienvenido a RocketInvest 🚀', `
            <div style="font-family: sans-serif; color: #333;">
                <h1 style="color: #307de8;">¡Hola, ${first_name}!</h1>
                <p>Tu cuenta ha sido creada exitosamente.</p>
                <p>Hemos depositado tu bono de <strong>$50,000 MXN</strong>.</p>
            </div>
        `);
...
// DATOS DE USUARIO (ME) - AHORA DEVUELVE EL NOMBRE
app.get('/api/auth/me', async (req, res) => {
    const token = req.headers.authorization?.split(' ')[1];
    if (!token) return res.status(401).json({ message: 'No token' });
    try {
        const decoded = jwt.verify(token, SECRET_KEY);
        const u = (await query('SELECT * FROM users WHERE id = $1', [decoded.id])).rows[0];
        ...
        res.json({
            id: u.id,
            email: u.email,
            // Enviamos los nombres al frontend
            first_name: u.first_name,
            last_name: u.last_name,
            availableBalance: parseFloat(u.balance),
            investedAmount: ti,
            currentValue: tc,
            profit: tc - ti,
            netWorth: parseFloat(u.balance) + tc,
...
app.post('/api/kyc/upload', upload.single('document'), async (req, res) => {
    ...
    try {
        ...
        if (!req.file) return res.status(400).json({ message: 'Falta documento' });
    ...
    } catch (e) { res.status(500).json({ message: 'Error KYC' }); }
});
...
// ... RESTO DE RUTAS (Invest, Portfolios, etc.) COPIAR DEL ANTERIOR O MANTENER ...
// Para ahorrar espacio asumo que las tienes, si no, avísame.
// IMPORTANTE: Asegúrate de que app.get('/api/portfolios'...) y demás sigan ahí.
