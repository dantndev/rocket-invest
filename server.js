...
// SERVIR ARCHIVOS ESTÁTICOS (CRÍTICO PARA QUE NO DE ERROR 404 EN JS)
...
const portfoliosBase = [
    { id: 1, name: "Alpha Tech Giants", provider: "BlackRock", ticker: "QQQ", risk: "Alto", returnYTD: 99.99, targetInvestors: 5000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "12 Meses", description: "Acceso grupal a las 100 tecnológicas más grandes." },
    { id: 2, name: "Deuda Soberana Plus", provider: "Santander", ticker: "SHV", risk: "Bajo", returnYTD: 8.12, targetInvestors: 10000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "3 Meses", description: "Bonos de gobierno con tasa preferencial." },
    { id: 3, name: "Energía Limpia Global", provider: "iShares", ticker: "ICLN", risk: "Medio", returnYTD: 14.50, targetInvestors: 2000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "24 Meses", description: "Infraestructura renovable global." },
    { id: 4, name: "Crypto Proxies", provider: "ProShares", ticker: "BITO", risk: "Alto", returnYTD: 145.20, targetInvestors: 1000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "6 Meses", description: "Exposición a futuros de Bitcoin." },
    { id: 5, name: "Bienes Raíces FIBRAs", provider: "Fibra Uno", ticker: "VNQ", risk: "Medio", returnYTD: 12.30, targetInvestors: 5000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "12 Meses", description: "Rentas comerciales diversificadas." },
    { id: 6, name: "Asian Tigers", provider: "HSBC Global", ticker: "VWO", risk: "Alto", returnYTD: 18.40, targetInvestors: 2000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "18 Meses", description: "Mercados emergentes de Asia." },
    { id: 7, name: "Deuda Corporativa USA", provider: "Vanguard", ticker: "LQD", risk: "Bajo", returnYTD: 4.50, targetInvestors: 8000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "6 Meses", description: "Bonos corporativos grado inversión." },
    { id: 8, name: "Gaming & eSports", provider: "VanEck", ticker: "ESPO", risk: "Alto", returnYTD: 32.10, targetInvestors: 3000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "12 Meses", description: "Entretenimiento digital." },
    { id: 9, name: "Oro Físico", provider: "SPDR", ticker: "GLD", risk: "Medio", returnYTD: 9.80, targetInvestors: 6000, baseInvestors: 0, baseAmount: 0, minInvestment: 1000, status: "open", lockUpPeriod: "Indefinido", description: "Resguardo en lingotes reales." }
];
...
app.get('/api/portfolios', async (req, res) => {
    try {
        const livePortfolios = await Promise.all(portfoliosBase.map(async (p) => {
            ...
            return { ...p, currentAmount: p.baseAmount + realMoney, investors: p.baseInvestors + realInvestors };
        }));
...
app.get('/api/auth/me', async (req, res) => {
    ...
    try {
        ...
        res.json({ email: user.email, availableBalance: parseFloat(user.balance), investedAmount: totalInvested, currentValue: totalCurrentValue, profit: totalCurrentValue - totalInvested, netWorth: parseFloat(user.balance) + totalCurrentValue });
...
app.get('/api/my-investments', async (req, res) => {
    ...
    try {
        ...
        const enriched = result.rows.map(inv => {
            ...
            return { id: inv.id, portfolioName: portfolio ? portfolio.name : 'Fondo', risk: portfolio ? portfolio.risk : 'N/A', investedAmount: amount, currentValue: amount * 1.015, profit: (amount * 1.015) - amount, date: inv.date };
        });
        res.json(enriched);
...
app.get('/api/market', async (req, res) => {
    try {
        const symbol = 'AAPL'; const interval = '1day'; const apikey = process.env.TWELVEDATA_API_KEY;
        ...
        if (response.data.values) {
            ...
            res.json({ prices: rawData.map(item => parseFloat(item.close)), dates: rawData.map(item => Math.floor(new Date(item.datetime).getTime() / 1000)) });
        } else { throw new Error("API Error"); }
    } catch (error) {
        const points = 30; const prices = []; const dates = []; let currentPrice = 180; 
        for (let i = 0; i < points; i++) { currentPrice *= (1 + (Math.random() * 0.06 - 0.025)); prices.push(currentPrice.toFixed(2)); dates.push(Math.floor(Date.now() / 1000) - ((points - 1 - i) * 24 * 60 * 60)); }
...
app.post('/api/invest', async (req, res) => {
    ...
    try {
        ...
        const existing = await query('SELECT id FROM investments WHERE userId = $1 AND portfolioId = $2', [user.id, parseInt(portfolioId)]);
        ...
        await query('INSERT INTO investments (userId, portfolioId, amount, date) VALUES ($1, $2, $3, $4)', [user.id, parseInt(portfolioId), investmentAmount, new Date().toISOString()]);
