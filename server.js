...
// EL CORREO DEL JEFE (Cámbialo si quieres, pero regístrate con este)
...
app.get('/api/admin/stats', async (req, res) => {
    ...
    try {
        ...
        // VALIDACIÓN DE JEFE
        if (decoded.email !== ADMIN_EMAIL) return res.status(403).json({ message: 'Acceso Denegado: No eres Admin' });

        // 1. Total Usuarios
        const usersRes = await query('SELECT COUNT(*) as count FROM users');
        const totalUsers = parseInt(usersRes.rows[0].count);

        // 2. Total Dinero en Plataforma (Suma de balances + Inversiones)
        ...
        // 3. Lista de Usuarios (Sin password)
    ...
    } catch (error) { console.error(error); res.status(500).json({ message: 'Error Admin' }); }
});


// --- RUTAS NORMALES ---
app.get('/api/portfolios', async(req,res)=>{try{const l=await Promise.all(portfoliosBase.map(async p=>{const s=await query('SELECT SUM(amount) as t FROM investments WHERE portfolioId=$1',[p.id]);const rm=parseFloat(s.rows[0].t||0);const c=await query('SELECT COUNT(DISTINCT userId) as c FROM investments WHERE portfolioId=$1',[p.id]);const ri=parseInt(c.rows[0].c||0);const tc=p.baseAmount+rm;return{...p,currentAmount:tc,totalTickets:Math.floor(p.targetAmount/p.minInvestment),soldTickets:Math.floor(tc/p.minInvestment),remainingTickets:Math.max(0,Math.floor(p.targetAmount/p.minInvestment)-Math.floor(tc/p.minInvestment)),investors:p.baseInvestors+ri}}));res.json(l)}catch(e){res.json(portfoliosBase)}});
