...
// Evitar caché en respuestas de API para que el contador se actualice al instante
...
app.get('/api/portfolios', async (req, res) => {
    try {
        const livePortfolios = await Promise.all(portfoliosBase.map(async (p) => {
            // 1. Consultar dinero real invertido por usuarios en la BD
            ...
            const soldTickets = Math.floor(totalCollected / p.minInvestment); // Tickets ocupados (Base + Real)
            ...
            return {
                ...
                remainingTickets: remainingTickets // ESTE ES EL DATO QUE USAREMOS EN EL FRONTEND
            };
        }));
...
// Resto de rutas estándar (Auth, Market, etc.) - Mantenidas igual
