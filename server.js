...
app.get('/api/portfolios', async (req, res) => {
    try {
        const livePortfolios = await Promise.all(portfoliosBase.map(async (p) => {
            ...
            return { ...p, currentAmount: p.baseAmount + realMoney, investors: p.baseInvestors + realInvestors, currentInvestors: p.baseInvestors + realInvestors };
        }));
...
app.post('/api/invest', async (req, res) => {
    ...
    console.log("Server Invest:", portfolioId, amount);
    try {
        const decoded = jwt.verify(token, SECRET_KEY);
