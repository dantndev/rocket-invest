...
// Ya no ponemos la clave aquí. El servidor la leerá de las variables de entorno.
...
// 1. STRIPE INTENT (CORREGIDO: Convierte a centavos y maneja errores)
...
app.post('/api/deposit', async (req, res) => {
    ...
    try {
        ...
        await query('UPDATE users SET balance = balance + $1 WHERE id = $2', [val, u.id]);
        await query('INSERT INTO transactions (userId, type, description, amount, date) VALUES ($1, $2, $3, $4, $5)', 
            [u.id, 'deposit', 'Depósito', val, new Date().toISOString()]);
...
app.post('/api/invest', async (req, res) => {
    ...
    try {
        ...
        if (ia < p.minInvestment || ia % p.minInvestment !== 0) return res.status(400).json({ message: `Múltiplos de $${p.minInvestment}` });
        if (parseFloat(u.balance) < ia) return res.status(400).json({ message: 'Saldo insuficiente' });
        
        const e = await query('SELECT id FROM investments WHERE userId=$1 AND portfolioId=$2', [u.id, p.id]);
        if (e.rows.length > 0) return res.status(400).json({ message: 'Ya eres socio' });
        ...
        await query('INSERT INTO transactions (userId, type, description, amount, date) VALUES ($1, $2, $3, $4, $5)', 
            [u.id, 'invest', `Compra ${s} cupos ${p.name}`, -ia, new Date().toISOString()]);
...
app.post('/api/auth/login', async(req,r)=>{const{email,password}=req.body;try{const u=(await query('SELECT * FROM users WHERE email=$1',[email])).rows[0];if(!u||!(await bcrypt.compare(password,u.password)))return r.status(400).json({message:'Credenciales inválidas'});const t=jwt.sign({id:u.id,email:u.email},SECRET_KEY,{expiresIn:'1h'});r.json({token:t})}catch{r.status(500).json({message:'Error'})}});
