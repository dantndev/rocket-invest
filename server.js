...
// Servir estáticos con cabeceras correctas para evitar caché agresivo
...
// --- CONFIGURACIÓN DE FONDOS (ENFOQUE: CAPITAL) ---
const portfoliosBase = [
    { 
        ...
        baseAmount: 7500000,    // Ya recaudado (Simulado)
        minInvestment: 1000,    // Costo por Ticket
    ...
    { id: 4, name: "Crypto Proxies", provider: "ProShares", ticker: "BITO", risk: "Alto", returnYTD: 145.20, targetAmount: 8000000, baseAmount: 1000000, minInvestment: 1000, lockUpPeriod: "6 Meses", description: "Exposición a futuros de Bitcoin." },
    { id: 5, name: "Bienes Raíces FIBRAs", provider: "Fibra Uno", ticker: "VNQ", risk: "Medio", returnYTD: 12.30, targetAmount: 20000000, baseAmount: 15000000, minInvestment: 1000, lockUpPeriod: "12 Meses", description: "Rentas comerciales diversificadas." },
    { id: 6, name: "Asian Tigers", provider: "HSBC Global", ticker: "VWO", risk: "Alto", returnYTD: 18.40, targetAmount: 5000000, baseAmount: 500000, minInvestment: 1000, lockUpPeriod: "18 Meses", description: "Mercados emergentes de Asia." },
    { id: 7, name: "Deuda Corporativa USA", provider: "Vanguard", ticker: "LQD", risk: "Bajo", returnYTD: 4.50, targetAmount: 10000000, baseAmount: 9000000, minInvestment: 1000, lockUpPeriod: "6 Meses", description: "Bonos corporativos grado inversión." },
    { id: 8, name: "Gaming & eSports", provider: "VanEck", ticker: "ESPO", risk: "Alto", returnYTD: 32.10, targetAmount: 3000000, baseAmount: 2800000, minInvestment: 1000, lockUpPeriod: "12 Meses", description: "Entretenimiento digital." },
    { id: 9, name: "Oro Físico", provider: "SPDR", ticker: "GLD", risk: "Medio", returnYTD: 9.80, targetAmount: 50000000, baseAmount: 41000000, minInvestment: 1000, lockUpPeriod: "Indefinido", description: "Resguardo en lingotes reales." }
];
...
app.get('/api/portfolios', async (req, res) => {
    try {
        const livePortfolios = await Promise.all(portfoliosBase.map(async (p) => {
            ...
            // 2. Total Recaudado (Base + Real)
            const totalCollected = p.baseAmount + realMoney;

            // 3. Cálculos de Tickets
            // Total de tickets disponibles en el fondo
            const totalTickets = Math.floor(p.targetAmount / p.minInvestment);
            // Tickets ya vendidos (ocupados)
...
app.post('/api/invest', async (req, res) => {
    ...
    try {
        ...
        if (existing.rows.length > 0) return res.status(400).json({ message: 'Ya tienes una posición activa en este fondo.' });
        ...
        await query('INSERT INTO investments (userId, portfolioId, amount, date) VALUES ($1, $2, $3, $4)', 
            [user.id, pid, investmentAmount, new Date().toISOString()]);
        await query('INSERT INTO transactions (userId, type, description, amount, date) VALUES ($1, $2, $3, $4, $5)', 
            [user.id, 'invest', `Entrada a ${portfolio.name}`, -investmentAmount, new Date().toISOString()]);
...
// Resto de Rutas (Auth, Market, etc.) - Simplificadas para espacio pero funcionales
